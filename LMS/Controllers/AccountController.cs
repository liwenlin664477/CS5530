...
namespace LMS.Controllers
{
  ...
  public class AccountController : CommonController
  {
    ...
    public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
    {
            ...
            using (Team4LMSContext db = new Team4LMSContext())
            {
                var querydID = (from d in db.Departments
                                where d.Subject.Equals(SubjectAbbrev)
                                select d.Subject).Distinct();
                switch (role)
                {
                    case "Administrator":
                        db.Add(new Administrators { UId = uid, FName = fName, LName = lName, Dob = DOB });
                        db.SaveChanges();
                        break;
                    case "Professor":
                        db.Add(new Professors { UId = uid, FName = fName, LName = lName, Dob = DOB, Subject = SubjectAbbrev });
                        db.SaveChanges();
                        break;
                    case "Student":
                        db.Add(new Students { UId = uid, FName = fName, LName = lName, Dob = DOB, Subject = SubjectAbbrev });
                        db.SaveChanges();
                        break;
                }
            }
    ...
    private IActionResult RedirectToLocal(string returnUrl)
    {
      ...
      else
      {
        ...
        /// <summary>
        /// Get the next largest UID from Students, Professors, and Administrators
        /// </summary>
        /// <returns>Next largest UID</returns>
        private string GetNextUid()
        {
            var sUid =
             from s in db.Students
             select s.UId.Substring(1);

            var pUid =
              from p in db.Professors
              select p.UId.Substring(1);

            var aUid =
              from a in db.Administrators
              select a.UId.Substring(1);

            List<string> UNids = aUid.Union(pUid).Union(sUid).ToList();
            UNids.Sort();


            int max = 0;
            max = int.Parse(UNids[UNids.Count - 1]) + 1;
            string format = "0000000.#";
            string uid = "u" + max.ToString(format);
